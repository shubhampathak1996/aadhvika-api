...
class UploadsController {
    ...
    static uploadSingle(req, res) {
        try {
            ...
            if (!req.file) {
                ...
                res.status(400).json({
                    success: false,
                    message: 'No file uploaded or invalid file type (only jpg, jpeg, png, gif allowed)',
                });
                return;
            }
            const file = req.file;
    ...
    static uploadVideo(req, res) {
        try {
            if (!req.file) {
                res.status(400).json({
                    success: false,
                    message: 'No video uploaded or invalid file type (mp4, mov, webm allowed)',
                });
                return;
            }
            const file = req.file;
    ...
    static async listUploads(req, res) {
        try {
            ...
            const files = await fs_1.default.promises.readdir(uploadsDir);
